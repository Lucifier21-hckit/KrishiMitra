  ...
  const send = async () => {
    ...
    try {
      const reply = await askClaude(
        newMessages,
        "You are KrishiMitra, a warm and practical AI farming assistant for small and marginal farmers in Chhattisgarh, India. Answer in simple, mixed Hindi-English (Hinglish) unless the user writes in pure English. Keep answers short, practical, and specific to Indian crops (rice, pulses, vegetables), soil, and government schemes where relevant. Never give definitive medical or pesticide dosage instructions without recommending confirmation from a local Krishi Vigyan Kendra or agriculture officer."
      );
...
function DiseaseTab() {
  ...
  const analyze = async () => {
    ...
    try {
      const reply = await askClaude(
        [{ role: "user", content: "Is fasal/patti ki photo dekhkar bataiye: 1) Kya rog ya samasya dikh rahi hai, 2) Kitni gambhir hai, 3) Kisan gharelu/saral upchaar kya kar sakta hai, 4) Kab agriculture officer se milna chahiye. Hindi-English mix mein, short bullet points mein jawab dein." }],
        "You are KrishiMitra's crop-health assistant. Examine the plant/leaf image for visible signs of disease, pest damage, or nutrient deficiency. Be clear that this is preliminary guidance, not a certified diagnosis, and recommend confirmation from the nearest Krishi Vigyan Kendra for serious cases. Never give exact pesticide dosages.",
        imageBase64
      );
      setResult(reply);
    } catch (e) {
